...
import { removeProject, removeProjectPin, renameProject, setProjectPin, unlockProject, listProjectSessions, revokeProjectSession, type ProjectSession } from "@/lib/pin";
...
export function ProjectSettings({ projectId, name, locked, onRenamed, onLockChange }: { projectId: string; name: string; locked: boolean; onRenamed: (name: string) => void; onLockChange: (locked: boolean) => void }) {
  ...
  return <>
    <div className="mt-4 grid gap-4 rounded-2xl border bg-card p-4 shadow-sm sm:grid-cols-2">
      ...
      <div className="space-y-2"><p className="flex items-center gap-2 text-sm font-medium"><ShieldCheck className="size-4" /> PIN project</p>{locked && <Input inputMode="numeric" maxLength={8} placeholder="PIN lama" value={oldPin} onChange={(e) => setOldPin(e.target.value.replace(/\D/g, ""))} />}<Input inputMode="numeric" maxLength={8} placeholder={locked ? "PIN baru (4-8 angka)" : "Buat PIN (4-8 angka)"} value={newPin} onChange={(e) => setNewPin(e.target.value.replace(/\D/g, ""))} /><div className="flex flex-wrap gap-2"><Button size="sm" className="rounded-xl" disabled={busy || newPin.length < 4} onClick={() => void run(async () => { await setProjectPin(projectId, newPin, locked ? oldPin : undefined); setOldPin(""); setNewPin(""); onLockChange(true); toast.success("PIN tersimpan"); })}><KeyRound className="size-4" />{locked ? "Ganti PIN" : "Aktifkan PIN"}</Button>{locked && <Button size="sm" variant="outline" className="rounded-xl" disabled={busy} onClick={() => setDisablePinConfirmOpen(true)}>Nonaktifkan PIN</Button>}<Button size="sm" variant="outline" className="rounded-xl" disabled={busy} onClick={() => void archiveProject()}>{archived ? <ArchiveRestore className="size-4" /> : <Archive className="size-4" />}{archived ? "Buka Arsip" : "Arsipkan"}</Button></div><p className="text-xs text-muted-foreground">PIN disimpan dalam bentuk terenkripsi di server dan tidak bisa ditampilkan kembali.</p></div>
