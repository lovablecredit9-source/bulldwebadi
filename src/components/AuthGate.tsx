...
export function AuthGate() {
  ...
  const openForgotPassword = () => {
    setShowForgotPassword(true); setResetStep("email"); setResetCode("");
    setNewPassword(""); setConfirmNewPassword(""); setResendCooldown(0);
  };
  const closeForgotPassword = () => {
    if (loading) return;
    setShowForgotPassword(false); setResetStep("email"); setResetCode("");
    setNewPassword(""); setConfirmNewPassword(""); setResendCooldown(0);
  };
    ...
    <div className="flex min-h-[70vh] items-center justify-center px-4 py-10">
      <div className="w-full max-w-md rounded-3xl border bg-card p-6 shadow-sm sm:p-8">
        ...
        <div className="mt-4 text-center">
          ...
          <p className="mt-2 text-sm text-muted-foreground">{mode === "login" ? "Login untuk masuk ke AI Builder." : "Daftar akun baru untuk langsung masuk ke AI Builder."}</p>
        </div>
      ...
      {showForgotPassword && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 px-4 backdrop-blur-sm"><div className="w-full max-w-md rounded-3xl border bg-card p-6 shadow-2xl sm:p-8">
        <div className="flex items-center gap-3"><div className="flex size-11 items-center justify-center rounded-xl bg-primary/10 text-primary"><Mail className="size-5" /></div><div><h2 className="text-xl font-bold">Lupa Password</h2><p className="text-sm text-muted-foreground">{resetStep === "email" ? "Kirim kode verifikasi ke email akun kamu." : "Masukkan kode dari email dan password baru."}</p></div></div>
        {resetStep === "email" ? <div className="mt-6 space-y-4"><div className="space-y-2"><Label htmlFor="reset-email">Email Akun</Label><Input id="reset-email" type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="nama@email.com" autoComplete="email" disabled={loading} /></div><Button type="button" onClick={sendResetCode} disabled={loading || resendCooldown > 0} className="h-11 w-full rounded-xl">{loading ? <Loader2 className="animate-spin" /> : <Mail />}Kirim Kode Verifikasi</Button><button type="button" onClick={closeForgotPassword} disabled={loading} className="w-full text-sm text-muted-foreground">Kembali ke Login</button></div> : <div className="mt-6 space-y-4">
