...
import { Bug, Bot, FolderTree, Home, History, Instagram, LogOut, Menu, MessageCircle, Moon, Puzzle, Search, Settings, ShieldCheck, Smartphone, Sun, Upload, Youtube, Loader2, WalletCards, CreditCard } from "lucide-react";
...
const NAV = [
  ...
  { to: "/wallet", label: "Deposit & Total Saldo", icon: WalletCards },
  { to: "/topup", label: "Top Up Kredit", icon: CreditCard },
  { to: "/account-history", label: "Riwayat Akun", icon: History },
...
function useTheme() {
  ...
  useEffect(() => { const stored = localStorage.getItem("adi-theme"); const isDark = stored ? stored === "dark" : true; setDark(isDark); document.documentElement.classList.toggle("dark", isDark); }, []);
  const toggle = () => { const next = !dark; setDark(next); document.documentElement.classList.toggle("dark", next); localStorage.setItem("adi-theme", next ? "dark" : "light"); };
  return { dark, toggle };
}
...
function Footer() {
  ...
  return <footer className="mt-12 border-t bg-card/50 px-6 py-8"><div className="mb-5 rounded-2xl border border-primary/20 bg-primary/5 px-4 py-3 text-center"><p className="text-sm font-semibold">Semua hasil project by Agung Adi</p><p className="mt-1 text-xs text-muted-foreground">ADI BUILDER BOT · Dibuat dan dikembangkan oleh Agung Adi</p></div><p className="text-base font-semibold">ADI BUILDER BOT</p><p className="mt-1 text-sm text-muted-foreground">Dibuat dan dikembangkan oleh Agung Adi</p><div className="mt-4 flex flex-wrap gap-2">{socials.map(({ href, label, icon: Icon }) => <Button key={href} asChild variant="outline" size="sm" className="rounded-full"><a href={href} target="_blank" rel="noreferrer"><Icon className="size-4" />{label}</a></Button>)}</div></footer>;
}
