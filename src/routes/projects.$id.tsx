...
function ChatTab({ projectId, model, files }: { projectId: string; model: string; files: ProjectFile[] }) {
  ...
  const renameChatPrompt = async (id: string, current: string) => { const title = typeof window === "undefined" ? null : window.prompt("Nama chat baru", current); if (!title?.trim()) return; await renameChat(id, title.trim()); setChats((prev) => prev.map((c) => (c.id === id ? { ...c, title: title.trim() } : c))); };
