...
function AnalyzeTab({ projectId, model, busy, setBusy }: { projectId: string; model: string; busy: string; setBusy: (v: string) => void }) {
  const [result, setResult] = useState<Analysis | null>(null);
  const [validation, setValidation] = useState<{ valid: boolean; issues: { level: string; file: string; message: string }[] } | null>(null);
  const run = async () => { setBusy("analyze"); try { setResult(await postJson<Analysis>("/api/ai/analyze-project", { projectId, model })); } catch (e) { toast.error(e instanceof Error ? e.message : "AI sedang mengalami gangguan."); } finally { setBusy(""); } };
  const validate = async () => { setBusy("validate"); try { setValidation(await postJson("/api/ai/validate-project", { projectId })); } catch (e) { toast.error(e instanceof Error ? e.message : "Validasi gagal."); } finally { setBusy(""); } };
  return <div className="grid gap-4"><div className="flex flex-wrap gap-2"><Button onClick={run} disabled={busy !== ""} className="rounded-xl">{busy === "analyze" ? <Loader2 className="size-4 animate-spin" /> : <Search className="size-4" />}AI Analyze</Button><Button onClick={validate} disabled={busy !== ""} variant="outline" className="rounded-xl">{busy === "validate" ? <Loader2 className="size-4 animate-spin" /> : <ShieldCheck className="size-4" />}Validate Extension</Button></div>{validation && <div className="rounded-2xl border bg-card p-4"><p className={`font-semibold ${validation.valid ? "text-primary" : "text-destructive"}`}>{validation.valid ? "✓ Valid" : "⚠ Problems Found"}</p><ul className="mt-2 space-y-1 text-sm text-muted-foreground">{validation.issues.map((i, n) => <li key={n}><span className="font-mono text-xs">{i.file}</span> — {i.message}</li>)}</ul></div>}{result && <div className="grid gap-3 rounded-2xl border bg-card p-4 text-sm"><div className="grid gap-1 sm:grid-cols-3"><p><span className="text-muted-foreground">Type:</span> {result.projectType}</p><p><span className="text-muted-foreground">Language:</span> {result.language}</p><p><span className="text-muted-foreground">Framework:</span> {result.framework}</p></div>{!!result.dependencies?.length && <p><span className="text-muted-foreground">Dependencies:</span> {result.dependencies.join(", ")}</p>}{!!result.errors?.length && <div><p className="font-semibold text-destructive">Errors</p><ul className="mt-1 space-y-1 text-muted-foreground">{result.errors.map((e, n) => <li key={n}><span className="font-mono text-xs">{e.file}</span> — {e.message}</li>)}</ul></div>}{!!result.warnings?.length && <div><p className="font-semibold">Warnings</p><ul className="mt-1 space-y-1 text-muted-foreground">{result.warnings.map((w, n) => <li key={n}><span className="font-mono text-xs">{w.file}</span> — {w.message}</li>)}</ul></div>}{!!result.recommendations?.length && <div><p className="font-semibold">Recommendations</p><ul className="mt-1 list-disc space-y-1 pl-5 text-muted-foreground">{result.recommendations.map((r, n) => <li key={n}>{r}</li>)}</ul></div>}{result.summary && <p className="text-muted-foreground">{result.summary}</p>}</div>}</div>;
}
...
function FixTab({ projectId, model, onApplied, busy, setBusy }: { projectId: string; model: string; onApplied: () => Promise<void>; busy: string; setBusy: (value: string) => void }) {
  ...
  const request = async () => { setBusy(mode); setLastAction(mode === "build" ? "add-feature" : mode); setProposal(null); setAnalysis(null); try { if (mode === "analyze") { setAnalysis(await postJson<Analysis>("/api/ai/analyze-project", { projectId, model, focus: instruction })); return; } const endpoint = mode === "build" ? "add-feature" : mode; setProposal(await postJson<Proposal>(`/api/ai/${endpoint}`, { projectId, instruction: mode === "build" ? `Bangun dan lengkapi kode berikut: ${instruction}` : instruction, model, images, attachments })); } catch (e) { toast.error(e instanceof Error ? e.message : "AI sedang mengalami gangguan."); } finally { setBusy(""); } };
  const apply = async () => { if (!proposal) return; setBusy("apply"); try { const res = await postJson<{ version: number }>("/api/project/apply", { projectId, label: instruction.slice(0, 80) || "Perubahan AI", action: lastAction, plan: proposal.plan, files: proposal.files.map((f) => ({ path: f.path, content: f.content, before: f.before, reason: f.reason })) }); toast.success(`Perubahan diterapkan. Backup: Version ${res.version}`); setProposal(null); await onApplied(); } catch (e) { toast.error(e instanceof Error ? e.message : "Perubahan tidak dapat diproses."); } finally { setBusy(""); } };
  return <div className="grid gap-4"><div className="rounded-2xl border bg-card p-4"><Select value={mode} onValueChange={(value) => setMode(value as typeof mode)} disabled={busy !== ""}><SelectTrigger className="mb-3 w-full sm:w-64"><SelectValue placeholder="Pilih pekerjaan AI" /></SelectTrigger><SelectContent><SelectItem value="build">Build</SelectItem><SelectItem value="fix-project">Fix</SelectItem><SelectItem value="add-feature">Tambah Fitur</SelectItem><SelectItem value="analyze">Analisis Error</SelectItem></SelectContent></Select><Textarea rows={4} value={instruction} onChange={(e) => setInstruction(e.target.value)} placeholder="Jelaskan kode, perbaikan, fitur, atau error yang ingin dianalisis." /><div className="mt-3"><ReferenceImages images={images} onChange={setImages} disabled={busy !== ""} /></div><div className="mt-3"><ReferenceFiles files={attachments} onChange={setAttachments} disabled={busy !== ""} /></div><div className="mt-3 flex flex-wrap gap-2"><Button onClick={() => void request()} disabled={busy !== ""} className="rounded-xl">{busy === mode ? <Loader2 className="size-4 animate-spin" /> : mode === "analyze" ? <Search className="size-4" /> : mode === "fix-project" ? <Wrench className="size-4" /> : <Sparkles className="size-4" />}{busy === mode ? "Sedang memproses…" : mode === "build" ? "Buat Rencana Build" : mode === "fix-project" ? "Buat Rencana Fix" : mode === "add-feature" ? "Buat Rencana Fitur" : "Analisis Error"}</Button></div>{(busy === "fix-project" || busy === "build") && <div className="mt-3"><AiWorkStatus kind="fix" /></div>}{busy === "add-feature" && <div className="mt-3"><AiWorkStatus kind="add-feature" /></div>}</div>{analysis && <div className="rounded-2xl border bg-card p-4 text-sm"><p className="font-semibold">Hasil Analisis</p>{analysis.summary && <p className="mt-1 text-muted-foreground">{analysis.summary}</p>}{!!analysis.errors?.length && <div className="mt-3 border-l-4 border-destructive pl-3"><p className="font-bold text-destructive">Error ditemukan</p>{analysis.errors.map((error, index) => <p key={`${error.file}-${index}`} className="mt-1 font-semibold text-destructive">{error.file}{error.line ? `:${error.line}` : ""} — {error.message}</p>)}</div>}{!analysis.errors?.length && <p className="mt-3 text-primary">Tidak ada error yang terdeteksi.</p>}</div>}{proposal && <div className="rounded-2xl border bg-card p-4"><p className="font-semibold">Rencana Perubahan</p><p className="mt-1 whitespace-pre-wrap text-sm text-muted-foreground">{proposal.plan}</p><div className="mt-4 space-y-4">{proposal.files.map((f) => <div key={f.path} className="rounded-xl border"><div className="border-b px-3 py-2"><p className="font-mono text-xs">{f.path}</p>{f.reason && <p className="text-xs text-muted-foreground">{f.reason}</p>}</div><div className="grid gap-2 p-3 md:grid-cols-2"><div><p className="mb-1 text-xs font-medium text-muted-foreground">Before</p><pre className="max-h-56 overflow-auto rounded-lg bg-muted p-2 text-[11px]">{f.before || "(file baru)"}</pre></div><div><p className="mb-1 text-xs font-medium text-primary">After</p><pre className="max-h-56 overflow-auto rounded-lg bg-muted p-2 text-[11px]">{f.content}</pre></div></div></div>)}</div><Button onClick={apply} disabled={busy !== ""} className="mt-4 rounded-xl">{busy === "apply" ? <Loader2 className="size-4 animate-spin" /> : <Check className="size-4" />}Terapkan Perubahan</Button><p className="mt-2 text-xs text-muted-foreground">Backup versi otomatis dibuat sebelum file ditimpa.</p></div>}</div>;
}

function ChatTab({ projectId, model, files }: { projectId: string; model: string; files: ProjectFile[] }) {
  const [chats, setChats] = useState<{ id: string; title: string }[]>([]); const [activeChat, setActiveChat] = useState(""); const [messages, setMessages] = useState<{ id: string; role: string; content: string }[]>([]); const [input, setInput] = useState(""); const [sending, setSending] = useState(false); const [photos, setPhotos] = useState<string[]>([]);
  const loadChats = useCallback(async () => { const c = await listChats(projectId); setChats(c); if (!activeChat && c[0]) setActiveChat(c[0].id); }, [projectId, activeChat]); useEffect(() => { void loadChats(); }, [loadChats]); useEffect(() => { if (activeChat) void listMessages(activeChat).then(setMessages); else setMessages([]); }, [activeChat]);
  const newChat = async () => { const c = await createChat(projectId); setChats((prev) => [c, ...prev]); setActiveChat(c.id); }; const send = async (text?: string) => { const message = (text ?? input).trim(); if (!message && !photos.length) return; const effectiveMessage = message || "Analisa dan rangkum seluruh foto ini, lalu tiru desainnya semirip mungkin pada project."; let chatId = activeChat; if (!chatId) { const c = await createChat(projectId, effectiveMessage.slice(0, 40)); chatId = c.id; setChats((prev) => [c, ...prev]); setActiveChat(c.id); } setSending(true); setMessages((m) => [...m, { id: `tmp-${Date.now()}`, role: "user", content: `${effectiveMessage}${photos.length ? `\n\n[${photos.length} foto dilampirkan]` : ""}` }]); try { const res = await postJson<{ reply: string }>("/api/ai/chat", { chatId, projectId, message: effectiveMessage, model, images: photos }); setMessages((m) => [...m, { id: `a-${Date.now()}`, role: "assistant", content: res.reply }]); setInput(""); setPhotos([]); } catch (e) { toast.error(e instanceof Error ? e.message : "AI sedang mengalami gangguan."); } finally { setSending(false); } };
  const rename = async (id: string) => { const title = window.prompt("Judul chat baru?"); if (!title) return; await renameChat(id, title); setChats((prev) => prev.map((c) => (c.id === id ? { ...c, title } : c))); }; const remove = async (id: string) => { await deleteChat(id); setChats((prev) => prev.filter((c) => c.id !== id)); if (activeChat === id) setActiveChat(""); };
  return <div className="grid gap-4 lg:grid-cols-[220px_1fr_220px]"><div className="rounded-2xl border bg-card p-3"><Button size="sm" onClick={newChat} className="w-full rounded-xl"><MessageSquarePlus className="size-4" />New Chat</Button><div className="mt-2 space-y-1">{chats.map((c) => <div key={c.id} className={`flex items-center gap-1 rounded-lg px-2 py-1 text-xs ${c.id === activeChat ? "bg-accent" : "hover:bg-accent/60"}`}><button className="flex-1 truncate text-left" onClick={() => setActiveChat(c.id)}>{c.title}</button><button onClick={() => rename(c.id)} aria-label="Rename"><Pencil className="size-3" /></button><button onClick={() => remove(c.id)} aria-label="Hapus"><Trash2 className="size-3" /></button></div>)}</div></div><div className="flex min-h-[420px] flex-col rounded-2xl border bg-card p-4"><div className="flex-1 space-y-3 overflow-auto">{messages.length === 0 && <p className="text-sm text-muted-foreground">Tanya apa saja tentang project ini. Context dikirim hanya dari file relevan.</p>}{messages.map((m) => <div key={m.id} className={`max-w-[90%] whitespace-pre-wrap rounded-2xl px-3 py-2 text-sm ${m.role === "user" ? "ml-auto bg-primary text-primary-foreground" : "bg-muted text-foreground"}`}>{m.content}{m.role === "assistant" && <button className="mt-2 flex items-center gap-1 text-xs text-muted-foreground" onClick={() => { void navigator.clipboard.writeText(m.content); toast.success("Disalin"); }}><Copy className="size-3" />Copy</button>}</div>)}{sending && <Skeleton className="h-10 w-2/3 rounded-2xl" />}</div><div className="mt-3 flex flex-wrap gap-2"><Button size="sm" variant="outline" onClick={() => send("Jelaskan struktur project ini.")}>Explain</Button><Button size="sm" variant="outline" onClick={() => send("Perbaiki error pada project ini.")}>Fix</Button><Button size="sm" variant="outline" onClick={() => send("Fitur apa yang sebaiknya ditambahkan?")}>Add Feature</Button></div><div className="mt-3 flex gap-2"><Input value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === "Enter" && send()} placeholder="Tulis pesan…" /><Button onClick={() => send()} disabled={sending}>{sending ? <Loader2 className="size-4 animate-spin" /> : <Send className="size-4" />}</Button></div><div className="mt-3"><ReferenceImages images={photos} onChange={setPhotos} disabled={sending} /></div></div><div className="rounded-2xl border bg-card p-3"><p className="mb-2 text-xs font-semibold text-muted-foreground">Project Files</p><div className="max-h-[420px] space-y-1 overflow-auto">{files.map((f) => <p key={f.path} className="truncate font-mono text-[11px]">{f.path}</p>)}</div></div></div>;
}
