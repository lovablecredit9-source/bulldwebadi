...
create or replace function public.wallet_set_pin(
  p_user_id uuid,
  p_pin_hash text,
  p_pin_salt text
) returns boolean
...
create or replace function public.wallet_get_pin(
  p_user_id uuid
) returns table(pin_hash text, pin_salt text)
language sql
security definer
set search_path = public
as $$
  select pin_hash, pin_salt from public.wallet_pins where user_id = p_user_id limit 1;
$$;
...
begin
  ...
  insert into public.wallet_transactions(
    ...
    p_user_id, p_type, p_amount, v_balance, p_reference_type, p_reference_id, p_description, p_created_by
  );
...
begin
  ...
  insert into public.wallet_transactions(
    ...
    p_user_id, 'admin_debit', -p_amount, v_balance, p_description, p_created_by
  );
...
begin
  ...
  if p_approve then
    ...
    update public.wallet_deposits
       set status = 'approved', reviewed_by = p_admin_id, reviewed_at = now(), admin_note = p_admin_note
     where id = p_deposit_id;
  else
    update public.wallet_deposits
       set status = 'rejected', reviewed_by = p_admin_id, reviewed_at = now(), admin_note = p_admin_note
     where id = p_deposit_id;
  end if;

  return jsonb_build_object('status', case when p_approve then 'approved' else 'rejected' end,
                            'balance', coalesce(v_balance, null));
end;
$$;
...
create trigger wallet_pins_updated_at before update on public.wallet_pins
for each row execute function public.set_wallet_updated_at();
